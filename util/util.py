...
# 无论是四维tensor还是三维tensor的list
# 全部转换成三维nparray的list, list中的每一项是一个三维nparray
def tensor2img(image_tensor, imtype=np.uint8, normalize=True):
    if isinstance(image_tensor, list):
        img_list = []
        for i in range(len(image_tensor)):
            img_list.append(tensor2img(image_tensor[i], imtype, normalize))
        return img_list
...
def tensor2label(label_tensor, n_label, imtype=np.uint8):
    ...
    if n_label == 0:
        return tensor2img(label_tensor, imtype)
...
class Colorize(object):
    def __init__(self, n=150):
        self.cmap = torch.from_numpy(label_colormap(n))
