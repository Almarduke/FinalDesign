...
import os
from spade.dataloader.label_ids import natural_colormap
...
def colorize(gray_image, n_label):
    # 将无符号整数（掩膜的编号）转换为二进制形式，编号最多为0-255
...
# 灰度语义分割图的tensor转换成有颜色的语义分割图
def tensor2colorlabel(label_tensor, opt, imtype=np.uint8):
    if opt.dataset == 'natural':
...
def use_gpu(opt):
    gpu_ids = get_gpu_ids(opt)
    return torch.cuda.is_available() and gpu_ids[0] >= 0


def get_device(opt, gpu_id=0):
    device_name = f'cuda:{gpu_id}' if use_gpu(opt) else 'cpu'
