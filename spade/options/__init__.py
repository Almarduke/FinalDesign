...
BASE_OPTION = {
    ...
    'gpu_ids': '-1',  # 使用的GPU，0 0,1,2, 0,2. CPU为-1
    'checkpoints_dir': 'spade/checkpoints',  # 模型保存的位置
    'images_dir': 'images',  # 图像保存的位置
    'model': 'spade',  # 使用的模型的名称
    'is_train': False,  # 是否是训练
    'continue_train': False,  # 继续训练，是则加载数据
    'current_epoch': 100,  # 如果是继续训练，从哪个epoch开始
...
NATURAL_DATASET_OPTION = {
    # 和数据输入部分相关
    'dataset_dir': 'dataset/refinedcoco/',  # 数据集所在的根目录
    'shuffle': True,  # 是否打乱数据集
    'flip': True,  # is_train=True & flip=True时，训练集中的图像有一半概率翻转，数据增强
    'thread_num': 0,  # dataloader用来读取数据集时的线程数，默认值为0
    'no_instance': True,  # 有无instance_map
    ...
    'n_label': 12,  # 标签数，包括无法识别的标签. 参见contain_dontcare_label.'
    'contain_dontcare_label': True,  # 无法识别的标签，对应255
    'output_nc': 3,  # 输出图像的通道数
    'total_epochs': 100,  # 总共有多少个epoch（包括learning rate decay的周期）
    'decay_epochs': 40,  # learning rate decay的周期数，10表示最后10个周期内发生decay
