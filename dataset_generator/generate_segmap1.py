...
from mycode import convert_segmap
...
_TARBALL_NAME = 'deeplabv3_xception_ade20k_train_2018_05_29-Copy1.tar.gz'
...
for index, img_file in enumerate(img_files):
    ...
    if not os.path.exists(f'{label_dir}/{label_file}'):
        try:
            original_im = Image.open(f'{img_dir}/{img_file}')
            resized_im, seg_map = MODEL.run(original_im)
            seg_map = convert_segmap(seg_map)
            seg_image = seg_map.astype(np.uint8)
            image_pil = Image.fromarray(seg_image)
            image_pil = image_pil.resize(original_im.size, Image.NEAREST)
            image_pil.save(f'{label_dir}/{label_file}')
        except Exception:
            continue
    running_time = time.time() - iter_start_time
    print(f'{index}: {running_time} sec', flush=True)
